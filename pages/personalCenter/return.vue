...
<script>
	export default {
		...
		onLoad(){
			let _this=this;
			_this.submitAjax("appmodule/appFile/selectLendFiles","","get",function(res){
 	...
 	<view class="detail">
 		...
 		<view class="menu-item" style="margin-top: 42px;">
 			<view v-for="m in menu" v-bind:id="m.id"   class="menu-item-table">
 				<view class="fileName sl" style="width: 36%;"><text>{{m.fkFileName}}</text></image>
 				</view>
 				<view class="fileLocation sl" style="width: 36%;"><text>{{m.shouldReturnTime}}</text></view>
 				<view class="operate" style="width: 28%;" @tap="back(m.fkFileId,m.shouldReturnTime)"><text>归还</text></view>
 			</view>
 			<br>
 		</view>

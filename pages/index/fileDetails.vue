<template>
	<view class="content">
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y" v-if="selectArchArr.length!=0"
		    @scroll="scroll"> 
	...
	export default { 
		...
		methods: { 
			...
			openshelf:function(localId) { 
				let _this = this;
				var data={};
				_this.submitAjax('storeroommodule/stoTbRegion/selectOpenMJJById', { localId }, 'GET',
					function(res) { 
						if (res.data.state && res.data.row){
							...
							+cols+'&section='+locate.divNum+'&layer='+locate.laysNum+'&direction='+dir,null, 'POST', function(res) { 
									uni.showToast({
										icon:'loading',
									    title: '打开架体中...',
									    duration: 1000
